...
/**
 * Slider is a copy of SlidingTray (http://aniqroid.sileria.com/doc/api/com/sileria/android/view/SlidingTray.html)
 ...
 * <p/>
 * // drawer
 * Slider drawer = new Slider( this, handle, content, Slider.Orientation.TOP );
 * </pre></blockquote>
 * <p/>
 * <p/>
...
public class Slider extends ViewGroup {
    ...
    /**
     * Construct a <code>Slider</code> object programmatically with the specified
     ...
     */
    public Slider(Context context, View handle, View content, Orientation orientation) {
        super(context);
    ...
    public Slider(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.Slider, defStyle, 0);

        int orientation = a.getInteger(R.styleable.Slider_orientation, Orientation.TOP.value);
        setOrientation(Orientation.getByValue(orientation));
        mHandlePos = Side.getByValue(orientation);
        mBottomOffset = (int) a.getDimension(R.styleable.Slider_bottomOffset, 0.0f);
        mTopOffset = (int) a.getDimension(R.styleable.Slider_topOffset, 0.0f);
        mAllowSingleTap = a.getBoolean(R.styleable.Slider_allowSingleTap, true);
        mAnimateOnClick = a.getBoolean(R.styleable.Slider_animateOnClick, true);


        int handleId = a.getResourceId(R.styleable.Slider_handle, 0);
        if (handleId == 0) {
            throw new IllegalArgumentException("The handle attribute is required and must refer "
                    + "to a valid child.");
        }

        int contentId = a.getResourceId(R.styleable.Slider_content, 0);
        if (contentId == 0) {
