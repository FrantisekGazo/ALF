package com.f3rog.alf.view.image;
...
import com.f3rog.alf.view.image.easing.Cubic;
import com.f3rog.alf.view.image.easing.Easing;
import com.f3rog.alf.view.image.graphics.FastBitmapDrawable;
import com.f3rog.alf.view.image.utils.IDisposable;
...
public abstract class ImageViewTouchBase extends ImageView implements IDisposable {
    ...
    protected InteractionListener _listener;

    public void setInteractionListener(InteractionListener listener) {
        this._listener = listener;
    }

    protected void onEdgeReached(Edge edge) {
        if (_listener != null) {
            _listener.onEdgeReached(edge);
        }
    }

    protected void onScaleChanged(float scale) {
        if (_listener != null) {
            _listener.onScaleChanged(scale);
        }
    }

    protected void onTaped() {
        if (_listener != null) {
            _listener.onTaped();
        }
    }

    protected void onDoubleTaped() {
        if (_listener != null) {
            _listener.onDoubleTaped();
        }
    }
    ...
    /**
     * Use this to change the {@link ImageViewTouchBase#setDisplayType(com.f3rog.alf.view.image.ImageViewTouchBase.DisplayType)} of
     * this View
     *
     * @author alessandro
     */
     ...
     *
     * @param bitmap the {@link android.graphics.Bitmap} to display
     ...
     * or the {@link com.f3rog.alf.view.image.ImageViewTouchBase.DisplayType} has been modified
     *
     * @param left
     * @param top
     * @param right
